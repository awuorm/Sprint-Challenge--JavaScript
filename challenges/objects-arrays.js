...
let contactInfo = graduates.map((arr) => `${arr.first_name} ${arr.email}`);
...
let lowerCase = zooAnimals.map((arr) => arr.animal_name.toLowerCase());
...
// }
let populationTotal = zooAnimals.reduce((acc, arr) => acc + arr.population,0);
